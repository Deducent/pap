...
tests := [?]Test {
	...
	{label = "write all bytes", func = write_to_all_bytes},
}
...
write_to_all_bytes :: proc(tester: ^repetition_tester, file_name: string) {
	for is_testing(tester^) {
		_ = file_name
		...
		begin_time(tester)
		for i := 0; i < len(tester.dest_buffer); i += 1 {
			tester.dest_buffer[i] = byte(i)
		}
		tester.process_byte_count = len(tester.dest_buffer)
		end_time(tester)
