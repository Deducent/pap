...
main :: proc() {
	...
	file_info, err := os.stat("./haversine/data.json")
	assert(err == nil, fmt.aprintf("failed to get file stats %v", err))

	profiler.time_block_start("open and read file", int(file_info.size))
	data, ok := os.read_entire_file("./haversine/data.json", context.allocator)
	profiler.time_block_end("open and read file")
	...
	sum: f64
	{
		profiler.time_block_start("Sum Loop", len(pairs) * size_of(json_parser.Json_value))
